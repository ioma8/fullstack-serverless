...
class ServerlessFullstackPlugin {
  constructor(serverless, cliOptions) {
    ...
    this.hooks = {
      ...
      "after:aws:deploy:deploy:updateStack": () =>
        this.serverless.pluginManager.run(["client", "deploy"]),
      "before:remove:remove": () =>
        this.serverless.pluginManager.run(["client", "remove"]),
      "before:aws:package:finalize:mergeCustomProviderResources": this.checkForApiGataway.bind(
        this
      ),
    };
  ...
  generateClient() {
    ...
    if (clientCommand && this.cliOptions["generate-client"] !== false) {
      ...
      return new BbPromise(
        this.performClientGeneration.bind(this, command, args, clientSrcPath)
      );
    } else {
  ...
  processDeployment() {
    ...
    return this.validateConfig()
      .then(() => {
        ...
        distributionFolder =
          this.options.distributionFolder || path.join("client/dist");
        ...
        invalidationPaths = invalidationPaths.map((path) =>
          path[0] === "/" ? path : "/" + path
        );
        ...
        if (this.cliOptions["delete-contents"] !== false) {
          deployDescribe.push(
            `- Remove all existing files from bucket '${bucketName}'`
          );
        }
        deployDescribe.push(
          ...
          return bucketUtils
            ...
            .then((exists) => {
              if (exists) {
                ...
                this.serverless.cli.log(
                  `Bucket does not exist. Run ${chalk.black(
                    "serverless deploy"
                  )}`
                );
            ...
            .then(() => {
              this.serverless.cli.log(`Success! Client deployed.`);
            });
        }
      ...
      .then(() => {
        if (this.cliOptions["invalidate-distribution"] === false) {
          ...
          return invalidateCloudfrontDistribution(
            this.serverless,
            invalidationPaths
          );
        }
  ...
  removeApiGatewayOrigin(baseResources) {
    this.serverless.cli.log(
      `ApiGatewayRestApi not found, removing orgin from CloudFront...`
    );
    const distributionConfig =
      baseResources.Resources.ApiDistribution.Properties.DistributionConfig;
    distributionConfig.Origins = _.filter(
      distributionConfig.Origins,
      (origin) => {
        return origin.Id !== "ApiGateway";
      }
    );
    distributionConfig.CacheBehaviors = _.filter(
  ...
  prepareOrigins(distributionConfig) {
    this.serverless.cli.log(
      `Setting ApiGateway stage to '${this.getStage()}'...`
    );
  ...
  prepareSinglePageApp(resources) {
    const distributionConfig =
      resources.ApiDistribution.Properties.DistributionConfig;
    const isSinglePageApp = this.getConfig("singlePageApp", false);
    if (isSinglePageApp) {
      this.serverless.cli.log(
        `Configuring distribution for single page web app...`
      );
      ...
      const statements =
        resources.WebAppS3BucketPolicy.Properties.PolicyDocument.Statement;
      resources.WebAppS3BucketPolicy.Properties.PolicyDocument.Statement = _.filter(
        statements,
        (statement) => {
  ...
  prepareS3(resources) {
    ...
    if (bucketName !== null) {
      ...
      this.serverless.cli.log(
        `Setting up '${resources.WebAppS3Bucket.Properties.BucketName}' bucket...`
      );
    }
  ...
  getBucketName(bucketName) {
    const stageBucketName = `${
      this.serverless.service.service
    }-${this.getStage()}-${bucketName}`;
  ...
  getConfig(field, defaultValue) {
    return _.get(
      this.serverless,
      `service.custom.fullstack.${field}`,
      defaultValue
    );
  }
